...
class BaseSpider(_BuildSchema[_R], Generic[_C, _R], abstract=True):
    """Базовый паук для парсинга, имеет лишь 2 абстрактных класса:

    :func:`get_page` - Получение страницы

    :func:`get_info` - Обработка URL-а

    Для работы, и системы автоматической инициализаии требуется указать базовый класс клиента для запросов :class:`BaseClient`
    Второй необязательный но желательный параметр это `_R`, указываете на возвращаемый тип данных.
    """
